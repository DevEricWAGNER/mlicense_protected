                    ...
                    <div class="flex flex-col p-3 bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                        ...
                        <form action="{{ route('license.store') }}" method="POST" class="flex flex-col gap-3">
                            ...
                            <div class="flex flex-col">
                                ...
                                <select id="script" name="script" class="bg-gray-600 rounded-md">
                                    <option value="0" selected class="bg-gray-600">Aucun</option>
                                    @foreach ($scripts as $script)
                                        <option value="{{ $script->id }}" class="bg-gray-600">{{ $script->script }}</option>
                                    @endforeach
                                </select>
                                @error('script')
                                    <span class="text-red-500">{{ $message }}</span>
